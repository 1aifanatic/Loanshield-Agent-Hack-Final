...
import { sendCustomerEvent } from "@/lib/dispute-service";
...
export async function POST(request: Request, context: Context) {
  try {
    ...
    const input = CustomerEventSchema.parse(body);
    const bundle = await sendCustomerEvent(externalDisputeId, input);
    return jsonOk(bundle, { status: 201 });
  } catch (error) {
    const apiError = toApiError(error, "Unable to send customer event");
